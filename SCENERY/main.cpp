...
void main()
{
	vec4 cool_position = model * vec4(in_position, 1.0);
	gl_Position = vec4(projection * cool_position.xyz, cool_position.w);
}
)";
...
void main()
{
	...
	vec4 shadow_coord = vec4(shadow_proj * position, 1.0);
	shadow_coord /= shadow_coord.w; // perspective divide
	shadow_coord = shadow_coord * 0.5 + vec4(0.5);
	bool in_shadow = texture(shadow, shadow_coord.xy).r < shadow_coord.z;
	vec3 light = ambient;
	if (in_shadow != true) {
		light = light + light_color * max(0.0, dot(normal, light_direction));
	}
...
std::pair<std::vector<vertex>, std::vector<std::uint32_t>> load_obj(std::istream & input)
{
	...
	for (std::string line; std::getline(input, line);)
	{
		...
		if (type == 'f')
		{
			...
			i0 = stoi(is0.substr(0, is0.find("/")));
			i1 = stoi(is1.substr(0, is1.find("/")));
			i2 = stoi(is2.substr(0, is2.find("/")));
			--i0;
			--i1;
			--i2;
		//	std::cout << i0 << " " << i1 << " " << i2;
...
int main() try
{
	...
	auto debug_v = create_shader(GL_VERTEX_SHADER, debug_vertex_shader_source);
	auto debug_f = create_shader(GL_FRAGMENT_SHADER, debug_fragment_shader_source);
	auto shadow_v = create_shader(GL_VERTEX_SHADER, shadow_vertex_shader_source);
	auto shadow_f = create_shader(GL_FRAGMENT_SHADER, shadow_fragment_shader_source);
	auto program = create_program(vertex_shader, fragment_shader);
	auto debug = create_program(debug_v, debug_f);
	auto shadow_prog = create_program(shadow_v, shadow_f);
	...
	GLuint shadow_model_location = glGetUniformLocation(shadow_prog, "model");
	GLuint shadow_proj_location = glGetUniformLocation(shadow_prog, "projection");

	GLuint main_proj_location = glGetUniformLocation(program, "shadow_proj");
	GLuint main_shadow_location = glGetUniformLocation(program, "shadow");

	GLuint debug_tex_location = glGetUniformLocation(debug, "tex");

	std::vector<vertex> vertices;
	std::vector<std::uint32_t> indices;
	{
		std::ifstream bunny_file(PRACTICE_SOURCE_DIRECTORY "/alduin-dragon.obj");
	...
	glm::vec3 light_X = glm::vec3(0, -1, 0);
	glm::vec3 light_Z = glm::vec3(1, 0, 0);
	glm::vec3 light_Y = glm::cross(light_X, light_Z);
	glm::mat3 projection_s = glm::transpose(glm::mat3(light_X, light_Y, light_Z));
	

	GLuint frameBuffer;
	glGenFramebuffers(1, &frameBuffer);
	glBindFramebuffer(GL_DRAW_FRAMEBUFFER, frameBuffer);
	glFramebufferTexture(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, shadow, 0);

	if (glCheckFramebufferStatus(GL_DRAW_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
		throw std::runtime_error("framebuffer did not compile");
	}
	...
	while (running)
	{
		...
		glm::vec3 light_X = glm::normalize(glm::vec3(light_Z.z, 0.0, -light_Z.x));
		glm::vec3 light_Y = glm::cross(light_X, light_Z);
		glm::mat3 projection_s = glm::transpose(glm::mat3(light_X, light_Y, light_Z));
		...
		glBindFramebuffer(GL_DRAW_FRAMEBUFFER, frameBuffer);
		glViewport(0.0, 0.0, shadow_map_res, shadow_map_res);
		...
		glUniformMatrix3fv(shadow_proj_location, 1, GL_FALSE, reinterpret_cast<float*>(&projection_s));
		glBindVertexArray(vao);
		...
		glUniformMatrix3fv(main_proj_location, 1, GL_FALSE, reinterpret_cast<float*>(&projection_s));
		glUniform1i(main_shadow_location, 0);

		glBindVertexArray(vao);
		glDrawElements(GL_TRIANGLES, indices.size(), GL_UNSIGNED_INT, nullptr);

		glUseProgram(debug);
		glUniform1i(debug_tex_location, 0);
		glActiveTexture(GL_TEXTURE0);
		glBindTexture(GL_TEXTURE_2D, shadow);
		glBindVertexArray(vao_v);
