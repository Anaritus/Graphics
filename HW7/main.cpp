...
int main() try
{
	...
	if (glCheckFramebufferStatus(GL_DRAW_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
		throw std::runtime_error("GL_DRAW_FRAMEBUFFER is not complete.");
	}
	...
	{
		std::ifstream dragon_file("D:/coding/Graphics/practice1/dragon.raw", std::ios::binary);
		...
		for (int i = 0; i < 4; i++) {
			...
			switch (i)
			{
				case 0:
					...
					break;
				case 1:
					projection = glm::ortho(-1.0f, 1.0f, (-1.0f * height) / width, (1.0f * height) / width, 0.1f, 100.0f);
					view = glm::rotate(view, view_angle + glm::pi<float>() / 2.f, { 0.f, 1.f, 0.f });
					break;
				case 2:
					projection = glm::ortho(-1.0f, 1.0f, (-1.0f * height) / width, (1.0f * height) / width, 0.1f, 100.0f);
					view = glm::rotate(view, view_angle + glm::pi<float>() / 2.f, { 0.f, 0.f, 1.f });
					break;
				case 3:
					projection = glm::ortho(-1.0f, 1.0f, (-1.0f * height) / width, (1.0f * height) / width, near, far);
					view = glm::rotate(view, view_angle + glm::pi<float>() / 2.f, { 1.f, 0.f, 0.f });
					break;
			}
			...
			glViewport(0, 0, width, height);
			glUseProgram(rectangle_program);
			glUniform2f(center_location, -0.5f + (i % 2) * 1.f, -0.5f + (i / 2) * 1.f);
